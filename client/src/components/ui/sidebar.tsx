...
export function Sidebar() {
  ...
  return (
    <aside
      className={cn(
        "sidebar fixed inset-y-0 left-0 z-20 w-64 bg-white/80 backdrop-blur-sm border-r border-border pt-16 pb-20 transform transition-all duration-300 ease-in-out lg:translate-x-0 shadow-sm flex flex-col h-full",
        isOpen ? "translate-x-0" : "-translate-x-full"
      )}
    >
      <div className="flex flex-col items-center justify-center py-6 mb-2 flex-shrink-0">
      ...
      <div className="flex-1 overflow-y-auto px-4 scrollbar-thin scrollbar-thumb-gray-300 scrollbar-track-transparent hover:scrollbar-thumb-gray-400">
        <div className="mb-4">
      ...
      <div className="absolute bottom-4 inset-x-0 px-6">
        <div className="flex items-center rounded-xl bg-primary/10 p-2 transition-colors hover:bg-primary/15">
          <button 
            onClick={() => { isOpen && close(); }} 
            className="flex items-center w-full justify-center text-xs font-medium text-primary hover:text-primary/80"
          >
